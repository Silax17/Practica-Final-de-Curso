...
public class SalaDAO {
	/**
	 * Clase DAO para gestionar las operacions CRUD de la entidad Sala
	 */
	// CREATE - Agregar una nueva sala a la bse de datos
	...
	//READ 2 - Obtener una sala por su ID (Steps)
	...
	// UPDATE - Actualizar los datos de una sala existente
	...
	// DELETE 1 solo sala
	/*
	 * public void eliminarSala(int idSala) throws SQLException { String sql =
	 * "DELETE FROM SALA WHERE id=?"; try (Connection con =
	 * ConexionBD.getConnection(); PreparedStatement st = con.prepareStatement(sql))
	 * { st.setInt(1, idSala); st.executeUpdate(); int filas = st.executeUpdate();
	 * if (filas > 0) { System.out.println("Sala eliminada con éxito."); } else {
	 * System.out.println("No se encontró una sala con ese ID."); }
	 * 
	 * } catch (SQLException e) { System.out.println("Error al eliminar sala");
	 * e.printStackTrace(); }
	 * 
	 * }
	 */
